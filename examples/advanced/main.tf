...
module "iks_base" {
  ...
  enable_vpc_cluster_version_upgrade = var.enable_vpc_cluster_version_upgrade
...
module "kube_audit" {
  depends_on = [module.iks_base] # Wait for the cluster to completely deploy.
  source     = "../../modules/kube-audit"
  # remove the above line and uncomment the below 2 lines to consume the module from the registry
  # source            = "terraform-ibm-modules/base-iks-vpc/ibm//modules/kube-audit"
  # version           = "X.Y.Z" # Replace "X.Y.Z" with a release version to lock into a specific release
  cluster_id                = module.iks_base.cluster_id
...
module "trusted_profile" {
  ...
  # Set up fine-grained authorization for `logs-agent` running in IKS cluster in `ibm-observe` namespace.
